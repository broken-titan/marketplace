...
**But check which standard the lab is accredited to**: AAVLD's requirements (SOP 1137 v1;
legacy name "AC1") clause 5.10.2.3 retains a per-page requirement — unique identification
"at the beginning and on each page" plus clear identification of the end — so for an
AAVLD-accredited veterinary laboratory this is a requirement, not a best practice. Both sit
under the same "unless the laboratory has valid reasons for not doing so" preamble. See
`poultry-veterinary.md`.
Treat it as a SHOULD and as one reasonable way to satisfy the first half of (d) — not
as a regulatory MUST. The end-of-report marker in (d) *is* a MUST.
...
The full sub-clause list, because earlier revisions of this file compressed several limbs
into 7.11.3 and mis-numbered the calculations rule. 7.11 **ends at 7.11.6**:
...
Consequences: retention is a per-engagement value rather than a platform constant — and
in some regimes it is anchored to a method lifecycle or a rolling last-entry date rather
than to the record's own creation, which a flat TTL cannot express (see
`poultry-veterinary.md`);
back-up and archive fall inside the accreditation requirement rather than being purely
infrastructural; and having a *defined* disposal control is required even if the
chosen policy is indefinite retention. Laboratories electing management-system
Option B satisfy this through an ISO 9001-conformant system instead.
