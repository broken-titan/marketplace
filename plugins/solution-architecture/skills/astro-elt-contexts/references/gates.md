...
Open a gate before writing files. Name the gate and its label (ASTRONOMER or DOMAIN). SKILL.md holds roles, defaults, and the quality bar.
...
THEN copy the URI from the Airflow UI after first parse. Format depends on adapter and Airflow 2 vs 3. See `astronomer.md`.
...
THEN `marts/{app}_{entity}` plus `contract_version` and `contract: enforced`. Tests (`unique`, `not_null`, `accepted_values`, `relationships`, source freshness) are gates, not comments.
...
Label DOMAIN applied to dbt Labs layers. Details in `layers.md`.
...
THEN `include/sql/{dialect}/...` via `include.sql_loader`. Dialect from `dialects.md`.
...
THEN follow `add-entity.md` in order. Confirm the REST path from the source API doc first.
