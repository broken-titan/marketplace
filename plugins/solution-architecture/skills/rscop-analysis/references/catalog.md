# RSCOP starter catalog

IDs in this file are the catalog. SAD Open Questions and ADR related-rows
must cite these IDs. Do not invent `O-E29`. P-E1 is application read
latency, not the data model (that is P-E11 / P-C14).

Envelope: if the engagement is not web/SaaS, skip P-C6–P-C11, P-C1/P-C2,
and S-C4/S-C5 per `envelope.md`.

S-C9 (breach-corpus / HIBP) and DAST-every-build are overlays
(`overlays.md`), not Defaults.

Assumed engagement class for Defaults: small-to-medium web / SaaS.
Scale latency, availability, and capacity up when the source describes
a larger or 24/7 system.

TLS / WCAG / RPO in this catalog are the numbers requirements and
defaults must match:

- Transport: TLS 1.3 required; 1.2 minimum (S-C1)
- Accessibility: WCAG 2.1 Level AA (P-C6) — web/SaaS only
- RPO transactional: at most 1 hour (R-E1); sourced/read-only: at most 24 hours (R-E2)
- RTO full system: at most 4 hours (R-E3)
...
| S-C9 | Password breach-corpus check | Whether passwords are checked against known-compromised lists | Off until flagged (overlay: HIBP / equivalent) | Overlay |
...
| S-E7 | Security scans in continuous integration | Static application security scans run during build | SAST on every build. DAST-every-build is an overlay | Default |
