...
Load this only when you are about to commit or push.

`/lean-gate-off` and `/lean-gate-on` toggle the project. On by default. Off writes `.lean-gate-off` so the commit/push hook allows; on clears it.
...
4. Does every new or changed switch, match, or key map fail closed, with no silent else?

If the diff fails any item, change the tree first. Do not commit or push the extra work.

When the commit/push hook blocks the command, finish this checklist, write `lean-gate.ok` in the repository git dir (`git rev-parse --git-dir`), and retry the same command. Do not write that file to skip the review.

`looks-good` may report this checklist as readiness or status. It does not run the hook, write `lean-gate.ok`, or change the toggle.
