...
description: >-
  Use during discovery calls, SOW drafting, or SAD authoring to walk NFR
  categories with HIT/MISS/DEFAULT/ASK. Grounded in the Solutions Architect
  Handbook and aligned with RSCOP.
---
A reusable NFR walk for discovery, SOW, and SAD work. Grounded in the *Solutions Architect Handbook* (Ch. 1, 2, 16) and aligned with RSCOP for negotiation conversations.

## How to run this

**When to use**

- **Discovery calls:** walk all 8 categories below. Do not skip a category because the client is talking features.
- **SOW drafting:** lock the answers. Anything still silent becomes a commitment you own.
- **SAD authoring:** these categories are the spine of Solution Management and Business Context.

**How to fill each category**

For every category, record four fields:

| Tag | Meaning |
|---|---|
| **HIT** | Client stated this. Quote or paraphrase; treat as locked. |
| **MISS** | Silent in scope. Nothing was committed. |
| **DEFAULT** | The target you will apply if they stay silent (from the lists below). |
| **ASK** | The question that turns a MISS into a HIT or an explicit override of the default. |

Walk HIT → MISS → DEFAULT → ASK in that order. A category is not done until all four are filled.

**Silence = you absorbed the risk.** A MISS is not "no requirement." If you apply a DEFAULT and leave it unflagged, you own that risk in the SOW.

**Where this feeds.** This skill produces the discovery matrix. Next artifact is **RSCOP Analysis**: one measurable row per requirement (explicit / implied / default / open). That analysis then feeds the **Solutions Architecture Document**.

**Default envelope.** Defaults assume a small-to-medium web/SaaS app at <10K users, on a Django/Rails-class web app or equivalent. Raise latency, availability, and throughput targets when the engagement is larger. Do not assume Django specifically.

## The two-tier model

The handbook splits requirements two ways:

**Functional Requirements (client-facing):** what the system *does*. Features, user flows, business processes. Defined by stakeholder interviews and user stories.

**Non-Functional Requirements (engineering-grade):** how the system *behaves*. Performance, security, reliability, etc. Defined by the architect and validated by the client. **NFRs are the category that slips when teams focus only on features — call them out explicitly every time.**

The matrix below is NFRs. Functional requirements get captured separately (user stories, use cases).

---

## Requirement Categories

Each category has: book grounding, client-side concern, engineering-side concern, common discovery questions, and a sensible default for a small-to-medium web/SaaS application. Defaults assume a Django/Rails-class web app or equivalent at <10K users; scale the targets up when the engagement is larger.

### 1. Performance & Scale

Book categories: Performance + Scalability + Capacity (RSCOP: P)
...
Book categories: Reliability + Availability + Recoverability (RSCOP: R)
...
- RPO ≤ 24h via daily backups; RTO ≤ 4h
...
Book category: Security & Compliance (RSCOP: S)
...
Book categories: Maintainability + Operability (RSCOP: O)
...
Book categories: Usability (RSCOP: not directly covered — usually shows up in client P or as a separate UX track)
...
- Accessibility: WCAG 2.1 AA — keyboard nav, alt text, sufficient contrast, labeled forms
...
Book category: Interoperability (touched in Ch. 2; SAD's "Integration Architecture" section)
...
Book categories: Extensibility + Portability (RSCOP: weakly covered under Cost — "minimal rework")
...
Book treats as a constraint layer, not a characteristic. RSCOP: C — also a constraint layer in practice.
...
The Solutions Architecture Document (handbook Ch. 16) has this structure. Each section maps to categories above:
...
1. **Discovery call** → fill the matrix (HIT/MISS/DEFAULT/ASK)
2. **RSCOP Analysis** → turn the matrix into measurable rows (explicit / implied / default / open)
3. **SOW** → lock the matrix into commitments
4. **Build phase week 1** → analysis becomes the SAD's "Business Context" and NFR sections
5. **Build phase ongoing** → SAD fills in Application / Data / Integration / Infrastructure / Security architecture sections
6. **Handoff** → SAD is the durable artifact the client inherits

## Two principles to internalize from the book

1. **NFRs slip when teams focus on features.** Most clients arrive with a feature list and zero NFRs. Your job is to bring the NFR conversation in deliberately. Silence on an NFR is not "no requirement" — it's "you've absorbed that risk."

2. **The architect bridges business and engineering.** Both perspectives are real and they want different things. The matrix forces both into the same artifact. Don't accept "the engineers will figure it out" as a closing answer on a business question, and don't accept "the business doesn't care about that" as a closing answer on a technical one.
