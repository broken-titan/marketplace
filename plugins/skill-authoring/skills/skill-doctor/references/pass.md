...
If the item uses disposition, carve-out, cannot state, without waiting, flag the cost, as a habit, style nit, name the X, Quality bar, courtroom evidence for why, with a name, stay silent, or a leftover closer of about 3-4 words, rewrite it to ordinary speech.
If the item uses `table` as the unnamed grid of cases ("one table," "in one table," "stop at the table"), rewrite it to many cases of one behavior in a single test, and leave table for a database table, an HTML table, or a documented table the sentence already named.
